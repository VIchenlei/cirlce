...
Page({
    ...
    getThemeList: function () {
        ...
        var url = that.data.url + '/api/services/app/theme/GetUserThemeList'
        var data = {
            circleId: that.data.circleId,
            userId: that.data.peruserId,
            pageIndex: 1,
            pageSize: 10
        };
        console.log(data)
            ...
            success: function (res) {
                ...
                if (res.data.success == true) {
                    ...
                    that.setData({
                        ...
                        circleCoverPicture: res.data.result.circleCoverPicture
                    });
                    wx.setNavigationBarTitle({
                        title: res.data.result.userNickName
                    })
                } else {
                    wx.showModal({
                        content: res.data.error.message,
    ...
    reportTap: function () {
        wx.navigateTo({
            url: '/pages/personalSpace/report/report?themeid=' + this.data.themeid + ''
        })
    },
    /**
     * 生命周期函数--监听页面加载
    ...
    onReachBottom: function () {
        ...
        //上拉加载更多圈子主体详情列表
        wx.request({
            url: that.data.url + "/api/services/app/theme/GetUserThemeList",
            method: "post",
            data: {
                circleId: that.data.circleId,
                userId: that.data.peruserId,
